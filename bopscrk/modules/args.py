...
class Arguments:
    def __init__(self):
        ...
        self.DEFAULT_CFG_FILE = os.path.abspath(
            os.path.join(os.path.dirname(os.path.abspath(__file__)), "../bopscrk.cfg")
        )
        # self.DEFAULT_CFG_FILE = './bopscrk.cfg'

        parser = argparse.ArgumentParser(
            description="Generates smart and powerful wordlists."
        )
        ...
        parser.add_argument(
            ...
            help="min length for the words to generate (default: {})".format(
                self.DEFAULT_MIN
            ),
        )
        ...
        parser.add_argument(
            ...
            help="max length for the words to generate (default: {})".format(
                self.DEFAULT_MAX
            ),
        )

        parser.add_argument(
            "-c", "--case", action="store_true", help="enable case transformations"
        )

        parser.add_argument(
            "-l", "--leet", action="store_true", help="enable leet transformations"
        )
        ...
        parser.add_argument(
            ...
            help="max amount of words to combine each time (default: {})".format(
                self.DEFAULT_N_WORDS
            ),
        )
        ...
        parser.add_argument(
            ...
            help="output file to save the wordlist (default: {})".format(
        ...
        parser.add_argument(
            ...
            help="specify config file to use (default: {})".format(
    ...
    def set_interactive_options(self):
        while True:
            min_length = input(
                "  {}[?]{} Passwords min length [{}] >>> ".format(
                    color.BLUE, color.END, self.DEFAULT_MIN
                )
            )
            if is_empty(min_length):
                self.min_length = self.DEFAULT_MIN
                break
            else:
                try:
                    ...
                    print(
                        "  {}[!]{} Min length should be an integer".format(
                            color.RED, color.END
                        )
                    )
        while True:
            max_length = input(
                "  {}[?]{} Password's max length [{}] >>> ".format(
                    color.BLUE, color.END, self.DEFAULT_MAX
                )
            )
            if is_empty(max_length):
                self.max_length = self.DEFAULT_MAX
                break
            else:
                try:
                    ...
                    if max_length < self.min_length:
                        print(
                            "  {}[!]{} Max should be greater or equal than min".format(
                                color.RED, color.END
                            )
                        )
                    else:
                    ...
                    print(
                        "  {}[!]{} Max length should be an integer".format(
                            color.RED, color.END
                        )
                    )
        ...
        while True:
            birth = input(
                "  {}[?]{} Birth date (DD/MM/YYYY) >>> ".format(color.BLUE, color.END)
            )
        ...
        others = input(
            "  {}[?]{} Some other relevant words (comma-separated) >>> ".format(
                color.BLUE, color.END
            )
        )

        leet = input(
            "  {}[?]{} Do yo want to make leet transforms? [y/n] >>> ".format(
                color.BLUE, color.END
            )
        )
        case = input(
            "  {}[?]{} Do yo want to make case transforms? [y/n] >>> ".format(
                color.BLUE, color.END
            )
        )
        ...
        while True:
            n_words = input(
                "  {}[?]{} How much words do you want to combine at most [{}] >>> ".format(
                    color.BLUE, color.END, self.DEFAULT_N_WORDS
                )
            )
            if is_empty(n_words):
                self.n_words = self.DEFAULT_N_WORDS
                break
            else:
                try:
                    n_words = int(n_words)
                    if n_words < 1:
                        print(
                            "  {}[!]{} Should be greater or equal than 1".format(
                                color.RED, color.END
                            )
                        )
                    else:
        ...
        self.outfile = input(
            "  {}[?]{} Output file [{}] >>> ".format(
                color.BLUE, color.END, self.DEFAULT_OUTPUT_FILE
            )
        )
        ...
        if not is_empty(birth):
            ...
            self.base_wordlist.append(
                (birth[2])[-2:]
            )  # Also add two last digits of the year
